...
- 에이전트별 품질 인사이트
- 설계/작성/수행 진행 중 에이전트 러닝 아바타 및 실시간 진행 UI
- 테스트데이터 에이전트 및 수행 중 테스트 데이터 생성/검증 중앙 큐 핸드오프
- 설계 품질 리뷰/점수
- TC 결과 REQ-ID 커버리지 UI
- 종합 품질 리뷰
- 자동 개선 루프
- BLOCKED 최후 판정 체크리스트 및 사유 코드화
...
진행 UI/BLOCKED 반영 기준:
- 설계/작성/수행 상태 페이지의 에이전트 러닝 아바타는 사용자 경험 개선 목적이므로 앱 UI diff로 반영해주세요.
- 수행 상세 페이지는 SSE/job stream 기반으로 진행률이 실시간 갱신되어야 하며, 단순 위아래 애니메이션만 있고 GOAL 방향으로 이동하지 않는 회귀가 없도록 확인해주세요.
- BLOCKED는 최후 판정 체크리스트와 사유 코드가 함께 적용되어야 합니다.
- selector 실패, 일시 로딩, 로그인 상태 꼬임, 팝업 가림처럼 재시도 가능한 항목은 즉시 BLOCKED로 확정하지 않도록 worker/external-worker 프롬프트 diff를 확인해주세요.
